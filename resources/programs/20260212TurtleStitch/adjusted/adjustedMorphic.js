...
// adjustes
function getCanvasEventPoint(event, canvas) {
    var posInDocument = getDocumentPositionOf(canvas),
        rect = canvas.getBoundingClientRect(),
        scaleX = rect.width ? (canvas.width / rect.width) : 1,
        scaleY = rect.height ? (canvas.height / rect.height) : 1;

    return new Point(
        (event.pageX - posInDocument.x) * scaleX,
        (event.pageY - posInDocument.y) * scaleY
    );
}
...
HandMorph.prototype.processMouseDown = function (event) {
    ...
    if (event.pageX) {
        // adjustes
        this.setPosition(getCanvasEventPoint(event, this.world.worldCanvas));
    }
...
HandMorph.prototype.processMouseMove = function (event) {
    ...
    // adjustes
    pos = getCanvasEventPoint(event, this.world.worldCanvas);
