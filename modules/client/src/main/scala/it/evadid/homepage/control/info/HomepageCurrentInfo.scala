...
case class HomepageCurrentInfo(fullInfo: FullInfo) {
  ...
  def workbookUserData: Option[WorkbookUserDataAnalyzer] = fullInfo.synchronized {
    ...
    else Some(WorkbookUserDataAnalyzer(fullInfo.loggerSystemInfo.contentControlLogger, fullInfo.contentControl.downloadToDisc, userInfo.get, workbookInfo.get))
  }
