...
import it.evadid.homepage.workbook.legacy.model.HtmlWorkbook
...
object JsonWorkbookRuntimeFactory {
  ...
  def toJsonString(workbook: HtmlWorkbook, pretty: Boolean = true): Option[String] =
    sourceJsonByWorkbookIdentity.get(System.identityHashCode(workbook)).map(_.toJson(pretty))

  private[factory] def register(workbook: HtmlWorkbook, source: JsonWorkbookFactory): Unit = {
