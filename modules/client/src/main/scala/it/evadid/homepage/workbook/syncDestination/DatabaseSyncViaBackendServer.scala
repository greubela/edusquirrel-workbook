...
case class DatabaseSyncViaBackendServer(dbName: String, hasKeyTable: Boolean) extends SyncDestination {
  ...
  override def clearAllValues(logger: SyncLogger, currentUser: Option[AllUserInfo], context: UsageContext): Future[SyncSuccess] = {
    clearValues(context, currentUser, None)
  }

  override def clearValues(logger: SyncLogger, currentUser: Option[AllUserInfo], context: SyncContext): Future[SyncSuccess] = {
    clearValues(context.toUsageContext, currentUser, Some(context.keyForSerialisation))
  }
