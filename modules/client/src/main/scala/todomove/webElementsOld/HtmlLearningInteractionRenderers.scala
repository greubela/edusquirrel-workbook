package todomove.webElementsOld

/*
object HtmlChoiceSelectionRenderer extends HtmlRenderFactory[ChoiceSelectionInteraction] {

  override protected def createRendering(interaction: ChoiceSelectionInteraction): HtmlWorkbookElement.WorkbookHtmlRendering = {
    ...
    // val ignored =   interaction.prompt.map(prompt => div(cls := "choice-selection-interaction__prompt", child.text <-- contentIdStringSignal(prompt))).toSeq,

    val cssStrChoice = s"choice-selection-interaction ${if interaction.allowMultiple then "choice-selection-interaction--multiple" else "choice-selection-interaction--single"}"
    ...
    RenderingLine(true, dom, cssStrChoice)
  }
