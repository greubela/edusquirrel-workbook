...
case class RGBColor(red: Int, green: Int, blue: Int, alpha: Int = 255) extends AppColor {
  ...
  override def toWebColor: WebColor = WebColorHexString(toHex(""))
...
object RGBColor {
  ...
  def fromPureSixDigitHex(sixDigitHex: String): RGBColor = {
    val r = Integer.parseInt(sixDigitHex.substring(0, 2), 16)
    val g = Integer.parseInt(sixDigitHex.substring(2, 4), 16)
    val b = Integer.parseInt(sixDigitHex.substring(4, 6), 16)
    RGBColor(r, g, b)
  }
