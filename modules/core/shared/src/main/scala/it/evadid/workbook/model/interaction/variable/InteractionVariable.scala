...
case class InteractionVariable[T](underlyingInteraction: WorkbookInteraction[T]) {
  ...
  def history: InteractionVariableHistory[T] = synchronized {
    innerState.now()
  }

  def serializedHistory: InteractionVariableHistorySerialized = synchronized {
  ...
  def addHistory(history: InteractionVariableHistorySerialized): Unit = synchronized {
    innerState.update(_.withAddedEvents(underlyingInteraction.serializer, history))
  }

  def syncToAll(forceFlush: Boolean = false): Unit = synchronized {
    ...
    println("[INFO] history '" + keyForSerialization + "' changed, synced to " + syncSources.now().size + " sources") //, current value: \n" + io.serialize(underlyingVar.now()) + ")")
  }
