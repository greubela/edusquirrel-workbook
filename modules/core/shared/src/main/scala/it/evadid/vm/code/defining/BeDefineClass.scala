...
case class BeDefineClass(
  ...
  override lazy val structureInfo: BeExpressionStructureInfo[?] = new BeExpressionStructureInfo[BeDefineClass](this) {
    override def withReplacedChildren(newChildren: Map[BeChildRole, BeExpression]): BeDefineClass = copy(
      attributes = attributes.zipWithIndex.map((attribute, index) =>
        newChildren.get(BeChildRole.AttributeInClass(index)).collect { case replacement: BeDefineVariable => replacement }.getOrElse(attribute)
      ),
      methods = methods.zipWithIndex.map((method, index) =>
        newChildren.get(BeChildRole.MethodInClass(index)).collect { case replacement: BeDefineFunction => replacement }.getOrElse(method)
      )
    )
    ...
    override def getChildrenAndExtension(myScope: BeScope): Seq[BeExpressionNode] =
      ...
      ) ++ methods.zipWithIndex.map((method, index) =>
