...
case class DisplayLangMapContent(override val elementId: String, content: LanguageMapContentId, contentType: LangMapContentIdType) extends WorkbookDisplayElement {
  override def toSerializableType: WorkbookElementFactory = toFactoryBase.withContentIdAdded("content", content).withElementAdded("role", contentType.contentRole.toString).withElementAdded("displayType", contentType.contentType.toString)
}
object DisplayLangMapContent {
 def fromFactory(f: WorkbookElementFactory): DisplayLangMapContent = {
