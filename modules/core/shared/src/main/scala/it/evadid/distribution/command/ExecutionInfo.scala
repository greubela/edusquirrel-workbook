...
trait ExecutionInfo {
  ...
  def toTyped[O](converter: TypeConverter[Map[String, String], O]): TypedExecutionInfo[O] = TypedExecutionInfo[O](command, result.map(_.toTyped(converter)), result, meta)

  def toTyped[O](converter: Map[String, String] => O): TypedExecutionInfo[O] = TypedExecutionInfo[O](command, result.map(_.toTyped(converter)), result, meta)
...
object ExecutionInfo {
  ...
  case class TypedExecutionInfo[T](command: ExecutionCommand, typedResult: Try[TypedExecutionResult[T]], result: Try[ExecutionResult], meta: Option[ExecutionHistory]) extends ExecutionInfo {
