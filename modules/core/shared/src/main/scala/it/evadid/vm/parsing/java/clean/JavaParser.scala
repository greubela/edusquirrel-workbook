...
object JavaParser {
  ...
  def decinteger[ctx: P]: P[JavaLiteral[BigInt]] =
    P((nonzero_digit ~~ (P("_").? ~~ digit).rep | P("0").rep(1, sep = P("_").?)).!).map(JAVA_INTEGER().createLiteralUnsafe)

  def bininteger[ctx: P]: P[JavaLiteral[BigInt]] = P("0" ~~ CharIn("bB") ~~ (P("_").? ~~ bindigit).rep(1)).!.map(JAVA_INTEGER_BIN().createLiteralUnsafe)

  def octinteger[ctx: P]: P[JavaLiteral[BigInt]] = P("0" ~~ CharIn("oO") ~~ (P("_").? ~~ octdigit).rep(1)).!.map(JAVA_INTEGER_OCT().createLiteralUnsafe)

  def hexinteger[ctx: P]: P[JavaLiteral[BigInt]] = P("0" ~~ CharIn("xX") ~~ (P("_").? ~~ hexdigit).rep(1)).!.map(JAVA_INTEGER_HEX().createLiteralUnsafe)

  def INTEGER[ctx: P]: P[JavaLiteral[BigInt]] = P(hexinteger | octinteger | bininteger | decinteger)

  def pointfloat[ctx: P]: P[String] = P(digitpart.? ~~ fraction | digitpart ~~ ".").!

  def expfloat[ctx: P]: P[String] = P((digitpart | pointfloat) ~~ exponent).!

  def FLOAT_NUMBER[ctx: P]: P[JavaLiteral[Double]] = P(expfloat | pointfloat).!.map(JAVA_FLOAT().createLiteralUnsafe)

  // def IMAG_NUMBER[ctx: P]: P[String] = P((FLOAT_NUMBER | digitpart) ~~ CharIn("jJ")).!

  def NUMBER[ctx: P]: P[JavaLiteral[?]] = P(FLOAT_NUMBER | INTEGER)

  def FALSE[ctx: P]: P[JavaLiteral[Boolean]] = P("False" ~~ !ID_CONTINUE).!.map(JAVA_BOOL().createLiteralUnsafe)

  def TRUE[ctx: P]: P[JavaLiteral[Boolean]] = P("True" ~~ !ID_CONTINUE).!.map(JAVA_BOOL().createLiteralUnsafe)

  //def NONE_LITERAL[ctx: P]: P[AstLiteral] = P(NONE.!).map(JAVA_NONE().literalFromStringUnsafe)

  def literal[ctx: P]: P[JavaLiteral[?]] = NUMBER | TRUE | FALSE | STRING_LITERAL

  def STRING_LITERAL[ctx: P]: P[JavaLiteral[String]] = P(STRING).!.map(JAVA_STRING().createLiteralUnsafe)

  def atomic_type[ctx: P]: P[JavaType[?]] =
    P("bool").!.map(_ => JAVA_BOOL())
      | P("Any").!.map(_ => JAVA_ANY())
      // | P("function").!.map(_ => JAVA_FUNCTION)
      | P("str").!.map(_ => JAVA_STRING())
      | P("int").!.map(_ => JAVA_INTEGER())
      | P("float").!.map(_ => JAVA_FLOAT())
      | NAME.map(str => JAVA_UNPARSABLE_TYPE(str))
*/
