    ...
    programmingLanguage match {
      case Python =>
        ...
        val methodBlocks = methods
          .map(method => indentLines(splitLines(method.getInLanguage(Python, humanLanguage)), indent))
          .filter(_.nonEmpty)
      ...
      case JavaScript =>
        ...
        val methodBlocks = methods
          .map { method =>
            val rendered = splitLines(method.getInLanguage(JavaScript, humanLanguage)) match {
      ...
      case Java =>
        ...
        val methodBlocks = methods.map { method =>
          ...
          val body = indentLines(splitLines(method.body.getInLanguage(Java, humanLanguage)), indent + indent)
