...
object AppColor {
  ...
  private val webColorNames: Map[String, String] = Map(
    "aliceblue" -> "#f0f8ff",
    "antiquewhite" -> "#faebd7",
    "aqua" -> "#00ffff",
    "aquamarine" -> "#7fffd4",
  ...
  def fromWebStyleString(webString: String): RGBColor = {
    if (webString.matches("#[0-9a-fA-F]{6}")) {
      ...
      val exWebNameCode = webColorNames.get(webString.toLowerCase())
      if (exWebNameCode.isDefined) fromWebStyleString(exWebNameCode.get)
      else throw new IllegalArgumentException("Unknown web color '" + webString + "'!")
    }
  }
