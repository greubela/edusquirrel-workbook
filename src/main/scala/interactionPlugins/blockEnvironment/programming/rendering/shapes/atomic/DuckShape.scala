...
object DuckShape extends BeShapePathBased {
  ...
  override protected def getPathBuilder(config: BeRendererConfig, bounds: Bounds[Double]): SvgPathBuilder[Double] = ShapeFactory.buildDuckShape(bounds)
  ...
  override protected def spaceBeforeChild(config: BeRendererConfig, childDim: Dimension[Double]): Dimension[Double] = {
    ...
    new Dimension[Double](leftOfText, minDim.height).increaseSize(config.paddingSmall)
  }
  ...
  override protected def spaceAfterChild(config: BeRendererConfig, childDim: Dimension[Double]): Dimension[Double] = {
    ...
    new Dimension[Double](rightOfText, minDim.height).increaseSize(config.paddingSmall)
  }
