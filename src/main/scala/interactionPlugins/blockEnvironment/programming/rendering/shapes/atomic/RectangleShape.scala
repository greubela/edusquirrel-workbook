...
import interactionPlugins.blockEnvironment.programming.rendering.shapes.ShapeFactory
 ...
 object RectangleShape extends  BeShapePathBased {
  ...
  override protected def getPathBuilder(config: BeRendererConfig, bounds: Bounds[Double]): SvgPathBuilder[Double] =  ShapeFactory.buildRectangle(bounds)

  override protected def spaceBeforeChild(config: BeRendererConfig, childDim: Dimension[Double]): Dimension[Double] = config.paddingSmall

  override protected def spaceAfterChild(config: BeRendererConfig, childDim: Dimension[Double]): Dimension[Double] = config.paddingSmall
