...
object LiteralShape extends BeShapePathBased {
  ...
  override protected def getPathBuilder(config: BeRendererConfig, bounds: Bounds[Double]): SvgPathBuilder[Double] = ShapeFactory.buildLiteralShape(bounds)
  ...
  override protected def getDefaultColorAmends(config: BeRendererConfig): Seq[L.Modifier[L.SvgElement]] =
    List(
