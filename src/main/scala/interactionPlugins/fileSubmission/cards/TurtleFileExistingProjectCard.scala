...
case class TurtleFileExistingProjectCard(workbookInfoVar: Var[WorkbookInfo], filename: String, existingProjectImg: ImageDescription, existingProject: URL) extends HtmlWorkbookElement {
  ...
  // private val loadedFileSignal: Var[Option[FullImage]] =


  /*
  calcedImgVar.signal.foreach(onNewCalcedImg => println("RECEIVED NEW: " + onNewCalcedImg))(unsafeWindowOwner)

  workbookInfoVar.signal.foreach(newValue => {
    calcImage(newValue.config.currentWorkbookLanguage)
  })(unsafeWindowOwner)
*/
  ...
  private val domElement: Element = div(
    ...
    /*div(
      cls := "preview-content",
      child <-- ImageStorage.loadFullImageIntoVar(existingProjectImg)(ExecutionContext.global).signal.map(_.map(_.imgSourceString).getOrElse("not loaded"))
    ),*/
    // HtmlHelper.toImgFromDataSrc("preview-content", loadedFileSignal.signal.map(_.map(_.imgSourceString))),
    // HtmlHelper.toImgFromDataSrc("preview-content", calcedImgVar.signal),
    downloadButton
  )
